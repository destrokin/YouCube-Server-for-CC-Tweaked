...
def download_video(
    ...
    returncode = run_with_live_output(
        [
            ...
            # Faster conversion:
            # - ordered dithering avoids the slower Floyd-Steinberg path
            # - default palette skips per-frame palette optimization
            "--ordered",
            "--default-palette",
...
def download(
    ...
    # This module is intentionally audio-only.
    # Never ask yt-dlp/Sanjuuni to create a video cache.
    is_video = False
    width = None
    height = None
    ...
    with TemporaryDirectory(prefix="youcube-") as temp_dir:
        ...
        if data.get("_type") == "playlist":
            ...
            if not entries:
                return {
                    "action": "error",
                    "message": "No playable media found for this request",
                }, []
