...
def download_video(
    ...
    source_files = [
        ...
        and name != "normalized_input.mp4"
    ]

    if not source_files:
        logger.error("No downloaded video source file was found.")
        run_coroutine_threadsafe(
            resp.send(
                dumps(
                    {
                        "action": "error",
                        "message": "No downloaded video source file was found!",
                    }
                )
            ),
            loop,
    ...
    normalized_file = join(temp_dir, "normalized_input.mp4")
    output_file = join(DATA_FOLDER, get_video_name(media_id, width, height))

    def ffmpeg_handler(line):
        logger.debug("[FFmpeg-Normalize] %s", line)
    ...
    run_coroutine_threadsafe(
        resp.send(
            dumps(
                {
                    "action": "status",
                    "message": "Normalizing video frames with FFmpeg ...",
                }
            )
        ),
        loop,
    ...
    normalize_code = run_with_live_output(
        [
            FFMPEG_PATH,
            "-y",
            "-i",
    ...
    if normalize_code != 0 or not exists(normalized_file):
        logger.warning(
            "FFmpeg normalization failed with code %s; using original source.",
            normalize_code,
        )
    ...
    # Sanjuuni raw output should contain:
    # line 1: format/header
    # line 2: FPS
    # line 3+: frame packets
    if exists(output_file):
        try:
            with open(output_file, "r", encoding="utf-8") as converted:
                header = converted.readline().rstrip("\n")
                fps_line = converted.readline().rstrip("\n")
                frame1 = converted.readline().rstrip("\n")
                frame2 = converted.readline().rstrip("\n")

            logger.info(
                "Sanjuuni raw check: header=%r fps=%r frame1=%s frame2=%s",
                header,
                fps_line,
                "YES" if frame1 else "NO",
                "YES" if frame2 else "NO",
            )

            if not frame1 or not frame2:
                logger.error("Sanjuuni generated fewer than two video frames.")
                ...
                run_coroutine_threadsafe(
                    resp.send(
                        dumps(
                            {
                                "action": "error",
                                "message": "Video conversion produced only one frame!",
                            }
                        )
                    ),
                    loop,
            ...
            logger.info("Video conversion validated: multiple frames present.")

        except Exception as exc:
            logger.error("Could not validate converted video: %s", exc)
...
def download(
    ...
    with TemporaryDirectory(prefix="youcube-") as temp_dir:
        yt_dl_options = {
            "format": "worstvideo*+worstaudio/worst" if is_video else "worstaudio/worst",
